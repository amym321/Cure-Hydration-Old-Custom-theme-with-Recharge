...
var ProductJs = function( item, product ) {
    ...
    productJs.config = {
      gallery:{
        galleryGroupByOptions: ''.split(',').map(function(s) { return s.trim() }).filter(function(v){return v!=='' && productOptions.indexOf(v)>-1}),
        galleryGroupMatchMode: 4, //1 Match Everything, 2 Match Any, 4 Exclusive OR
        slickMainOptions: {
          slidesToShow: 1,
          slidesToScroll: 1,
          speed: 700,fade: true,waitForAnimate: false,
          arrows: false,
          swipe: false,
        ...
        slickThumbsOptions: {
          ...
          nextArrow: '<svg aria-hidden="true" focusable="false" role="presentation" class="icon icon-arrow-right right-arrow" viewBox="0 0 32 32"><path fill="#000000" d="M7.667 3.795l1.797-1.684L24.334 16 9.464 29.889l-1.797-1.675L20.731 16z"/></svg>',responsive: [
            {
              breakpoint: 600,
              settings: {
                slidesToShow: 6,
    ...
    productJs.slickFilterThumbs = function(slickOptions, objectOptions, optionsContainer, gallery) {
      ...
      if(groupOptionProperties.length>0) {
        ...
        gallery.find('.slideVisible:eq(0) img').addClass('lazyload');
    ...
    var switchMainImage = function(mediaId){
        if(item.find('.main-images .slick-track').length>0) {
          ...
          var nextImage = item.find('.main-images .slide:eq('+index+') img');
